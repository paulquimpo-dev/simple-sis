...
| 2026-08-19 | 2.1 | Student scaffold TODO sequencing revision | Deferred `TODO-PT-CREATE-01` and `TODO-PT-UPDATE-02` to Guided PT Lab 5 so `ModelState` is not required before Module 7; valid CREATE and UPDATE persistence paths remain in Labs 2 and 3. | TODO traceability, documentation consistency, restore/build | Removes a hidden theoretical prerequisite from the executable student workflow without changing the final teacher-reference behavior |
